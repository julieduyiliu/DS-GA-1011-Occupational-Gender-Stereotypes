...
# df = pd.read_csv(Config.BERT_FILE, sep="\t", encoding="utf8", header=None, names=["pos"])
# preds2_with_control = list(np.array(df["pos"]).astype("float32"))
# preds2 = preds2_with_control[:800]

# # Check t-test for BERT predictions
# (t2, prob2, diff2) = utils.ttest(preds2)
# results.append([t2, prob2, diff2])

# GPT-4
preds3 = list(np.array(pd.read_csv(Config.GPT_FILE, encoding="utf8")).reshape(-1,))
(t3, prob3, diff3) = utils.ttest(preds3)
results.append([t3, prob3, diff3])

print("Results:", results)
np.savetxt(Config.RESULTS_FILE, np.array(results), 
           header="t prob f-m (models: logreg bilstm gpt-4o-mini)")
print("\n")
