...
class GPTFineTuner:
    ...
    def prepare_data(self, X, y):
       ...
       system_prompt = """You are a sentiment analysis model.
                        Task: Classify text sentiment with a score between 0 and 1.
                        Rules:
                        1. Output must be a decimal between 0 and 1 (e.g., 0.2, 0.8)
                        2. Consider:
                            - Language tone
                            - Emotional content
                            - Context clues
                        3. Lower scores (closer to 0) = more negative
                        4. Higher scores (closer to 1) = more positive
                        5. Neutral = 0.5
                        6. No explanation needed
                        7. Focus on overall sentiment"""
       formatted_data = []
...
def main():
    ...
    # Load existing model ID
    with open(Config.GPT_MODEL_FILE, 'r') as f:
        model_id = json.load(f)['model_id']
    
    # Get dev set predictions
    # dev_predictions = fine_tuner.get_predictions(model_id, dev_data)
    # results from previous run
    dev_accuracy = 0.9690366972477065
    ...
    test_predictions = fine_tuner.get_predictions(model_id, test_data)
