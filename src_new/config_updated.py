...
class Config:
    ...
    GLOVE_DIR = Path("/content")  # Set the path to the directory of the GN-GloVe file
    GLOVE_FILE = Path(join(GLOVE_DIR, "1b-vectors300-0.8-0.8.txt"))  # Path to the GN-GloVe file
    LSTM_MODEL_FILE = Path(join(RUNS_DIR, "lstm.h5"))
    LOGREG_FILE = Path(join(RUNS_DIR, "logreg.txt"))
    LSTM_FILE = Path(join(RUNS_DIR, "lstm.txt"))
    BERT_MODEL_FILE = Path(join(RUNS_DIR, "bert_model.pth")) #newly added
    DISCRIMINATOR_FILE = Path(join(RUNS_DIR, "discriminator.pth")) #newly added
